...
function KpiCard({ label, value, icon: Icon, iconColor, accentColor, iconBg, loading }) {
  return (
    <div className="relative bg-white rounded-[18px] shadow-[0_2px_20px_rgba(0,63,135,0.07)] border border-[rgba(0,63,135,0.07)] overflow-hidden px-4 sm:px-6 py-4 sm:py-5 transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(0,63,135,0.13)]">
      <div className="absolute top-0 left-0 right-0 h-1 rounded-t-[18px]" style={{ background: accentColor }} />
      <div className="flex justify-between items-start">
        <div className="flex-1 min-w-0 pr-2">
          <p className="text-[10px] sm:text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5 sm:mb-2">
            {label}
          </p>
          {loading ? (
            <div className="h-6 w-24 rounded-lg bg-gradient-to-r from-slate-100 via-slate-200 to-slate-100 bg-[length:400px_100%] animate-[shimmer_1.4s_infinite]" />
          ) : (
            <div className="text-lg sm:text-2xl font-extrabold text-[#002960] break-all">{value}</div>
          )}
        </div>
        <div
          className="w-10 h-10 sm:w-12 sm:h-12 rounded-[12px] sm:rounded-[14px] flex items-center justify-center shrink-0"
...
function ChartCard({ title, subtitle, badge, children }) {
  return (
    <div className="bg-white rounded-[18px] shadow-[0_2px_20px_rgba(0,63,135,0.07)] border border-[rgba(0,63,135,0.06)] p-4 sm:p-6 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(0,63,135,0.11)] min-w-0">
      <div className="flex justify-between items-start mb-4 sm:mb-5">
        <div className="flex-1 min-w-0 pr-2">
          <div className="font-bold text-[#002960] text-[14px] sm:text-[15px] truncate">{title}</div>
          <div className="text-xs text-slate-400 mt-0.5 truncate">{subtitle}</div>
        </div>
        <span className="inline-flex items-center gap-1 bg-[#EEF4FF] text-[#003F87] text-[10px] sm:text-[11px] font-bold px-2 sm:px-3 py-1 rounded-full whitespace-nowrap shrink-0">
          {badge}
        </span>
      </div>
      {children}
    ...
    <div className="px-4 sm:px-8 py-4 sm:py-6 max-w-full overflow-x-hidden">
      ...
      <div className="flex gap-2 sm:gap-2.5 mb-4 sm:mb-5 flex-wrap">
        <button
          ...
          className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-bold text-white
        ...
        >
          {exporting ? (
            <><Loader2 size={13} className="animate-spin" /> Membuat PDF...</>
          ) : (
      ...
      <div className="bg-[#F0F4FA] p-1 rounded-2xl">
        ...
        {/* ── Header ── */}
        <div
          className="rounded-2xl px-4 sm:px-6 py-4 sm:py-5 mb-4 sm:mb-5 flex items-center justify-between relative overflow-hidden"
        ...
        >
          ...
          <div className="flex items-center gap-2.5 sm:gap-3.5 relative z-10 flex-1 min-w-0">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white/15 border border-white/15 flex items-center justify-center shrink-0">
              <Landmark size={22} color="white" strokeWidth={1.8} />
            </div>
            <div className="min-w-0">
              <div className="text-white font-extrabold text-base sm:text-xl truncate">BNI Life Insurance</div>
              <div className="text-white/65 text-[11px] sm:text-[13px] mt-0.5 font-normal truncate">
          ...
          <div className="text-right relative z-10 shrink-0 hidden sm:block">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mb-2">
      ...
      {/* ── Footer ── */}
      <p className="text-center text-xs text-slate-400 pt-4 sm:pt-5 pb-2">
